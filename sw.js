...
const SHELL_CACHE='ark-shell-v3';
const MEDIA_CACHE='ark-media-v3';
...
self.addEventListener('fetch',(event)=>{
  ...
  if(isMediaRequest(url)){
    event.respondWith((async()=>{
      ...
      try{
        // Le immagini/video ospitati su un altro dominio (es. Supabase Storage)
        // arrivano spesso come risposta "opaca" (cross-origin, no-cors): in quel
        // caso res.ok è sempre false anche se il download è andato a buon fine,
        // quindi qui NON filtriamo su res.ok — salviamo comunque la risposta.
        const res=await fetch(req);
        if(res){
